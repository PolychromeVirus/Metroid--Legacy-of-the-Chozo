...
if (!is_undefined(pending_choice)) {
    _camera_cross_player_interaction = pending_choice.kind == "raid"
...
if (network_lobby_active) {
    if (net_role == "client" && net_join_field == "address") {
        ...
        net_player_name = string_copy(keyboard_string, 1, 24);
...
if (title_menu_active) {
    ...
    if (mouse_check_button_pressed(mb_left)
    && !is_undefined(_hover_region)) {
        if (_hover_region.kind == "title_name_field") {
            title_name_editing = true;
            ...
            title_name_editing = false;
            batch_seed_editing = false;
            if (_hover_region.action == "title_toggle_faction_starters") {
...
if (game_state.game_mode == "batch") {
    ...
    repeat (500) {
        ...
        batch_match_steps += 1;
        if (batch_match_steps >= 200000) {
            batch_match_invalid_reason =
                "AI decision safety limit reached at "
                + string(batch_match_steps) + " steps; pending "
...
&& !is_undefined(_hover_region)) {
    ...
    if (_net_is_action || _net_is_choice) {
        ...
        && (_hover_region.kind == "character"
            ...
            _net_input_kind =
                _hover_region.instance.controller == game_state.active_player
                    ? "character"
                    : "opponent_character";
        }
...
if (mouse_check_button_pressed(mb_left)) {
    if (is_undefined(_hover_region)) {
        ...
        if (_hover_region.enabled) {
            ...
            switch (_hover_region.action) {
                ...
                case "raid_use_0":
                    activate_raid_ability_index(0);
                    break;

                case "raid_use_1":
                    activate_raid_ability_index(1);
                    break;

                case "raid_use_2":
                    activate_raid_ability_index(2);
                    break;
    ...
    && (_hover_region.kind == "character"
        || _hover_region.kind == "opponent_character")) {
        raid_toggle_character(_hover_region.index, false);
    } else if (!is_undefined(pending_choice)
    && pending_choice.kind == "raid"
    && pending_choice.stage == "defenders"
    && pending_choice.interaction_mode == "contributors"
    && !is_undefined(_hover_region.instance)
    && _hover_region.instance.controller == 1 - game_state.active_player
    && (_hover_region.kind == "character"
        || _hover_region.kind == "opponent_character")) {
        raid_toggle_character(_hover_region.index, true);
    } else if (!is_undefined(pending_choice)
    && pending_choice.kind == "raid"
    && pending_choice.interaction_mode == "abilities"
    && !is_undefined(_hover_region.instance)
