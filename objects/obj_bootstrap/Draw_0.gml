...
var _player_one_palette = game_state.game_mode == "hotseat"
    ? game_state.active_player == 0
    : game_state.view_player == 0;
ui_color_background = _player_one_palette
    ? make_color_rgb(8, 13, 22)
    : make_color_rgb(22, 12, 9);
ui_color_header = _player_one_palette
    ? make_color_rgb(12, 22, 34)
    : make_color_rgb(38, 21, 15);
...
ui_color_panel = _player_one_palette
    ? make_color_rgb(18, 29, 43)
    : make_color_rgb(48, 28, 20);
ui_color_panel_alt = _player_one_palette
    ? make_color_rgb(24, 38, 55)
    : make_color_rgb(61, 37, 27);
ui_color_line = _player_one_palette
    ? make_color_rgb(53, 78, 98)
    : make_color_rgb(126, 76, 48);
ui_color_title = _player_one_palette
    ? make_color_rgb(95, 235, 220)
    : make_color_rgb(255, 177, 83);
ui_color_text = make_color_rgb(232, 240, 246);
ui_color_muted = _player_one_palette
    ? make_color_rgb(157, 177, 194)
    : make_color_rgb(207, 165, 135);
...
ui_color_hud_surface = make_color_rgb(31, 35, 41);

var _draw_panel = function(_x1, _y1, _x2, _y2, _label) {
    draw_set_alpha(0.54);
    draw_set_color(make_color_rgb(22, 142, 166));
    draw_rectangle(_x1, _y1, _x2, _y2, false);
    draw_set_alpha(0.9);
    draw_set_color(make_color_rgb(84, 229, 242));
...
var _draw_modal_panel = function(_x1, _y1, _x2, _y2, _label) {
    ...
    draw_rectangle(_x1, _y1, _x2, _y2, false);
    draw_set_color(make_color_rgb(75, 84, 94));
    draw_rectangle(_x1, _y1, _x2, _y2, true);
    draw_set_color(ui_color_title);
    draw_set_font(FNT_METROID);
...
_draw_action_button("TEST TOOLS", "test_open", _log_left, 8,
...
_draw_action_button(
    ...
    _log_left + (_watch_header ? 148 : 110),
    8,
...
_draw_action_button(
    ...
    "camera_lock_toggle",
    _log_left + (_watch_header ? 254 : 236),
    8,
    ...
    for (var _opponent_card_index = 0;
        ...
        && pending_choice.kind == "raid") {
            ...
            var _opponent_raid_ability =
                ...
                && card_has_raid_ability(_opponent_card);
        ...
        && array_length(_opponent_card.cargo) > 0) {
            var _opponent_cargo = _opponent_card.cargo[0];
            var _opponent_cargo_w = 24;
            var _opponent_cargo_h = 34;
            var _opponent_cargo_x = _opponent_bounds.x
                + _opponent_bounds.width - _opponent_cargo_w;
            var _opponent_cargo_y = _opponent_bounds.y
...
var _shop_panel_top = _sr_planet_y - 178;
...
var _shop_row_1_y = _shop_panel_top + 38;
...
for (var _active_layout_index = 0;
     ...
     _active_layout_index++) {
        ...
        && _zone_kind == "character"
        && ((pending_choice.stage == "attackers"
            && _active_card.controller == game_state.active_player)
            || (pending_choice.stage == "defenders"
            && _active_card.controller == 1 - game_state.active_player))
        && _active_card.ready) {
            draw_set_color(
                raid_array_contains(
                    pending_choice.stage == "attackers"
        ...
        && pending_choice.interaction_mode == "abilities"
        && card_has_raid_ability(_active_card)) {
...
&& (game_state.phase != "containment"
    ...
    if (!is_undefined(_context_anchor_region)) {
        ...
        _button_x1 = clamp(
            ...
            _main_left + 4,
            _screen_width - _context_total_w - 4
        );
...
if (!is_undefined(pending_choice)) {
    ...
    var _transmission_x1 = 10;
    var _transmission_y1 = _content_top + 8;
    var _transmission_x2 = min(_main_right - 10, _transmission_x1 + 460);
    var _transmission_body_width =
        (_transmission_x2 - _transmission_x1) - 20;
    var _transmission_line_spacing = 24;
    draw_set_font(FNT_METROID);
    var _transmission_body_height = string_height_ext(
        _pending_prompt,
        _transmission_line_spacing,
        _transmission_body_width
    );
    var _transmission_y2 = min(
        _screen_height - 10,
        _transmission_y1 + max(
            108,
            34 + _transmission_body_height + 12
        )
    );
    draw_set_alpha(0.62);
    draw_set_color(c_black);
    ...
    || pending_choice.kind == "raid_cargo") {
        ...
        var _raid_score_bottom = _raid_score_top + 32;
        var _raid_score_mid =
        ...
        draw_text(
            _raid_score_left + 8,
            (_raid_score_top + _raid_score_bottom) * 0.5,
            "ATTACK"
        );
        draw_set_halign(fa_right);
        draw_text(
    ...
    } else if (pending_choice.kind == "queen_event"
    ...
    || pending_choice.stage == "defenders")) {
        if (pending_choice.interaction_mode == "abilities") {
            var _raid_source = get_ability_source(
                pending_choice.ability_source_kind,
                pending_choice.ability_source_index
            );
            if (is_undefined(_raid_source)) {
                ...
                var _raid_legal_indices =
                    get_raid_ability_indices(_raid_source);
                var _raid_legal_count = min(
                    2,
                    array_length(_raid_legal_indices)
                );
                for (var _raid_choice_index = 0;
                     _raid_choice_index < _raid_legal_count;
                     _raid_choice_index++) {
                    var _raid_actual_index =
                        _raid_legal_indices[_raid_choice_index];
                    _draw_action_button(
                        _raid_abilities[_raid_actual_index].label,
                        "raid_use_" + string(_raid_actual_index),
                        _raid_choice_index == 0
                            ? _button_x1
                            : _button_x2,
                        _action_bottom - 94,
                        _raid_legal_count == 1
...
if (array_length(load_errors) > 0 || array_length(setup_errors) > 0) {
    draw_set_color(ui_color_error);
    draw_text(_log_left + 12, _diagnostic_y, "State errors are present.");
    _diagnostic_y += 25;
} else {
    draw_set_color(ui_color_success);
...
draw_set_color(ui_color_text);
for (var _event_index = array_length(game_state.event_log) - 1;
     _event_index >= 0 && _diagnostic_y < _hud_content_bottom - 28;
     _event_index--) {
    draw_text_ext(
        _log_left + 12,
        _diagnostic_y,
...
// In network play, keep the local palette stable and communicate remote
// control by dimming only the local play area.
if (game_state.game_mode == "network"
&& game_state.active_player != network_local_player
...
if (game_state.phase == "game_over") {
    ...
    var _end_h = min(650, _screen_height - 80);
...
if (title_menu_active) {
    draw_set_alpha(1);
    draw_set_color(make_color_rgb(5, 10, 17));
    draw_rectangle(0, 0, _screen_width, _screen_height, false);
    ...
    draw_set_color(make_color_rgb(12, 24, 36));
    draw_rectangle(
        _title_panel_x,
        _title_panel_y,
    ...
    draw_set_color(make_color_rgb(48, 91, 105));
    draw_rectangle(
        _title_panel_x,
        _title_panel_y,
        _title_panel_x + _title_panel_w,
    ...
    draw_set_color(ui_color_panel_alt);
    draw_rectangle(
        _screen_width * 0.5 - 210,
        _title_panel_y + 292,
        _screen_width * 0.5 + 210,
        _title_panel_y + 340,
        false
    );
    draw_set_color(title_name_editing
        ? ui_color_selected
        : ui_color_line);
    draw_rectangle(
        _screen_width * 0.5 - 210,
    ...
    _draw_action_button(
        faction_starters_enabled
            ? "FACTION STARTERS: ON"
            : "FACTION STARTERS: OFF",
        "title_toggle_faction_starters",
        _screen_width * 0.5 - 210,
        _title_panel_y + 352,
        420,
        32,
    ...
    draw_set_halign(fa_left);
    draw_set_valign(fa_top);
    var _title_button_gap = 24;
    var _title_button_w = min(
        330,
        floor((_title_panel_w - 120 - _title_button_gap) * 0.5)
    );
    var _title_button_y = _title_panel_y + 420;
    var _title_button_left = _screen_width * 0.5
        - _title_button_w - (_title_button_gap * 0.5);
    _draw_action_button(
        "2 PLAYER HOTSEAT",
        "title_hotseat",
        _title_button_left,
    ...
    _draw_action_button(
        "PLAY VS AI",
        "title_ai",
        _screen_width * 0.5 + (_title_button_gap * 0.5),
        _title_button_y,
        _title_button_w,
        62,
        true
    ...
    var _title_utility_gap = 12;
    var _title_utility_w = floor(
        ((_title_button_w * 2) + _title_button_gap
            - (_title_utility_gap * 2)) / 3
    );
    var _title_utility_left = _title_button_left;
    _draw_action_button(
        "WATCH AI VS AI",
        "title_ai_watch",
        _title_utility_left,
    ...
    _draw_action_button(
        "BATCH TESTS",
        "title_batch",
        _title_utility_left + _title_utility_w + _title_utility_gap,
        _title_button_y + 164,
        _title_utility_w,
        46,
        true
    ...
    _draw_action_button(
        ...
        "title_regression",
        _title_utility_left
            + ((_title_utility_w + _title_utility_gap) * 2),
        _title_button_y + 164,
        _title_utility_w,
        46,
        true
    ...
    if (batch_menu_active) {
        ...
        draw_text(
            ...
            + string(
                batch_game_options[batch_game_option_index] * 21
            ) + " pairs / "
            + string(
                batch_game_options[batch_game_option_index] * 36
            ) + " games"
        );
        draw_set_halign(fa_left);
        _draw_action_button(
        ...
        && variable_global_exists("loc_batch_state")) {
            ...
            var _matrix_profiles = ["", "GF", "SP", "CZ", "BH", "PZ"];
            ...
            for (var _matrix_row = 0; _matrix_row < 6; _matrix_row++) {
                draw_set_halign(fa_center);
                draw_set_color(_matrix_profile_color(
